...
from app.clients.exceptions import NHTSABadRequestError, NHTSAHTTPError, NHTSANotFoundError, NHTSATimeoutError, NHTSAUnavailableError
...
def test_unavailable_returns_503(mock_analysis):
    mock_analysis.side_effect = NHTSAUnavailableError(
...
def test_timeout_returns_504(mock_analysis):
    mock_analysis.side_effect = NHTSATimeoutError(
...
def test_bad_request_returns_400(mock_analysis):
    ...
    response = client.get(
        "/vehicles/TOTO/BIDULE/2018/complaints/ranking"
    )

    assert response.status_code == 400
    assert response.json() == {
        "detail": "NHTSA API rejected the complaint request"
    }
...
def test_not_found_returns_404(mock_analysis):
    mock_analysis.side_effect = NHTSANotFoundError(
    ...
    assert response.json() == {
        "detail": "NHTSA resource was not found"
    }
...
def test_http_error_returns_502(mock_analysis):
    ...
    assert response.json() == {
        "detail": "NHTSA API returned an unexpected HTTP error"
    }